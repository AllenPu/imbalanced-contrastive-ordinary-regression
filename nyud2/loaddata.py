...
from nyud2.nyu_transform import *
from scipy.ndimage import convolve1d
from nyud2.util import get_lds_kernel_window
...
class depthDataset(Dataset):
    def __init__(self, data_dir, csv_file, mask_file=None, transform=None, args=None):
        ...
        self.mask = torch.tensor(
            np.load(mask_file), dtype=torch.bool) if mask_file is not None else None
        self.transform = transform
        self.bucket_weights = self._get_bucket_weights(
            args) if args is not None else None
    ...
    def __getitem__(self, idx):
        ...
        image_name = os.path.join(
            self.data_dir, '/'.join(image_name.split('/')[1:]))
        depth_name = os.path.join(
            self.data_dir, '/'.join(depth_name.split('/')[1:]))
...
def getTrainingFDSData(args, batch_size=64):
    ...
    dataloader_training = DataLoader(transformed_training, batch_size,shuffle=False, num_workers=8, pin_memory=False)
...
def getTestingData(args, batch_size=64):
    ...
    transformed_testing = depthDataset(data_dir=args.data_dir,
                                       csv_file=os.path.join(
                                           args.data_dir, 'nyu2_test.csv'),
                                       mask_file=os.path.join(
                                           args.data_dir, 'test_balanced_mask.npy'),
                                       transform=transforms.Compose([
                                           Scale(240),
